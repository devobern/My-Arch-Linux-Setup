...
do_mount()
{
    ...
    sudo -u nicolin DISPLAY=:0 DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1000/bus notify-send "Device ${DEVICE} mounted at ${MOUNT_POINT}"
    # Start backup script
    sudo -u nicolin ./usr/local/bin/backup.sh "${MOUNT_POINT}"
}
